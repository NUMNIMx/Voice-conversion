...
class SVC_Model:
    ...
    def load_model(self, model_path):
        ...
        try:
            ...
            # Try loading the model with torch.load
            try:
                self.model = torch.load(model_path, map_location=self.device)
                self.model.eval()
                self.is_loaded = True
                print(f"Successfully loaded model with torch.load from {model_path}")
                return True
            except Exception as e:
                print(f"Failed to load with torch.load: {str(e)}, trying torch.jit.load...")
                
                # If torch.load fails, try torch.jit.load
                self.model = torch.jit.load(model_path, map_location=self.device)
                self.model.eval()
                self.is_loaded = True
                print(f"Successfully loaded model with torch.jit.load from {model_path}")
                return True
        except Exception as e:
    ...
    def convert(self, audio_path, output_path, pitch_shift=0):
        ...
        try:
            ...
            sf.write(output_path, output, self.sampling_rate)
...
class SVC_Trainer:
    ...
    def train(self, processed_files, model_path, epochs=100, batch_size=8, update_callback=None):
        ...
        try:
            ...
            # สร้างโมเดลและแก้ไขอื่นๆ เหมือนเดิม
            ...
            # แก้ไขการบันทึกโมเดล: ใช้ torch.save แทน torch.jit.script
            try:
                # วิธีที่ 1: บันทึกแบบทั่วไป
                torch.save(self.model, model_path)
                print(f"Model saved successfully with torch.save to {model_path}")
            except Exception as e:
                print(f"Failed to save model with torch.save: {str(e)}, trying torch.jit.script...")
                # วิธีที่ 2: ถ้าวิธีที่ 1 ล้มเหลว ใช้ torch.jit.script
                torch_script_model = torch.jit.script(self.model)
                torch_script_model.save(model_path)
                print(f"Model saved successfully with torch.jit.script to {model_path}")
